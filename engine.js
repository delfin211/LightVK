...
(function(w) {
		...
		case 'object':
			if (Object.prototype.toString.apply(value) === '[object Array]') {
				...
				for (let i = 0, len = value.length; i < len; i++) {
					newValue[i] = walklet(value[i], clean);
				};
			} else {
				newValue = {};
				for (let k in value) {
					if (Object.hasOwnProperty.call(value, k)) {
						newValue[k] = walklet(value[k], clean);
					};
				};
			};
			break;
	...
	function applyMethod(strData, self, origin) {
		getEnv(function(env) {
			let data = env.json.parse(strData);
			let method = data[0];
			...
			if (method) {
				...
				for (let i = 0, len = data[1].length; i < len; i++) {
					if (data[1][i] && data[1][i]._func) {
						...
						data[1][i] = walklet(data[1][i], true);
					};
				};
				setTimeout(function() {
	...
	w.fastXDM = {
			...
			getEnv(function(env) {
				for (let method in this.methods) {
					if (this.methods[method] instanceof w.fastXDM.Server) {
						let proxyFromServer = this;
						let proxyToServer = this.methods[method];
		...
		onMessage: function(e) {
			...
			let key = data.substr(0, 5);
			if (handlers[key]) {
				let self = handlers[key][1];
	...
	w.fastXDM.Client.prototype.callMethod = w.fastXDM.Server.prototype.callMethod = function() {
		...
		for (let i = 0, len = args.length; i < len; i++) {
			if (typeof(args[i]) === 'function') {
				...
				let func        = args[i];
				let funcName    = '_func' + this.funcsCount;
		...
		waitFor(this, 'caller', function() {
			w.fastXDM.on(this.id, function() {
				getEnv(function(env) {
					let xdm = this;
					let proxyToServer = w.fastXDM.getProxyToServer(this.key, method);
	...
	if (w.postMessage) {
		...
		env.send = function(xdm, strData) {
			let win = (xdm.frame ? xdm.frame.contentWindow : xdm.caller);
			if (win) {
				try {
...
VK.init = function(success, failure, ver) {
	...
	let activeEvents = ['mousemove', 'keydown'];
	for (let i = 0; i < 0; i++) {
...
VK.loadParams = function(q) {
	if (typeof q == 'Object') VK.params = q;
		...
		let tmp = q.substr(q.indexOf('?') + 1).split('&');
		let i = tmp.length;
		while (i--) {
			let v = tmp[i].split('=');
...
(function() {
	try {
		...
		let domain = script.src.match(/^https?:\/\/((?:\w+\.)*vk.com)/)[1];
		if (domain) {
...
VK.Widgets = (function() {
	let obj = {};
	let widgetlist = ['Comments', 'CommentsBrowse', 'Recommended', 'Post', 'Like', 'Poll', 'Group', 'Auth', 'Subscribe', 'ContactUs', 'Ads', 'AllowMessagesFromCommunity', 'CommunityMessages'];
		...
		obj[f] = function() {
			...
			if (!VK._openApiAttached) {
				VK.callMethod('_getAppInfo', function(data) {
					let baseDomain = VK._domain && VK._domain.base && VK._domain.base.match(/^(\w+\.)*vk.com$/) ? VK._domain.base : 'https://vk.com';
					VK._apiId = data[0];
...
VK.MD5 = function(n) {
	let j = function(o, r) {
				let q = (o & 65535) + (r & 65535),
						p = (o >> 16) + (r >> 16) + (q >> 16);
				return (p << 16) | (q & 65535);
		},
		g = function(o, p) {return (o << p) | (o >>> (32 - p))},
		k = function(w, r, p, o, v, u) {return j(g(j(j(r, w), j(o, u)), v), p)},
		a = function(q, p, w, v, o, u, r) {return k((p & w) | ((~p) & v), q, p, o, u, r)},
		h = function(q, p, w, v, o, u, r) {return k((p & v) | (w & (~v)), q, p, o, u, r)},
		c = function(q, p, w, v, o, u, r) {return k(p ^ w ^ v, q, p, o, u, r)},
		m = function(q, p, w, v, o, u, r) {return k(w ^ (p | (~v)), q, p, o, u, r)},
		b = function(A, u) {
			let z = 1732584193, y = -271733879, w = -1732584194, v = 271733878, r, q, p, o;
			A[u >> 5] |= 128 << ((u) % 32);
			A[(((u + 64) >>> 9) << 4) + 14] = u;
			for (let t = 0, s = A.length; t < s; t += 16) {
				r = z;
				q = y;
				p = w;
				o = v;
		...
		d = function(o) {return l(b(e(o), o.length * 8))},
		i = function(q) {
...
if (!VK.xdConnectionCallbacks) {
	...
	VK.Cookie = {
		_domain: null,
		load: function() {
			let cookie = document.cookie.match('\\bvk_app_' + VK._apiId + '=([^;]*)\\b');
			let session;
		...
		setRaw: function(val, ts, domain, time) {
			...
			let exp = time ? (new Date().getTime() + time * 1000) : ts * 1000;
	...
	if (!VK.Api) {
		VK.Api = {
			...
			makeRequest: function(url, cb) {
				let xhr = VK.Api.createRequest('GET', url);
				if (!xhr) {
					return false;
				};
			...
			checkMethod: function(method, params, cb, queryTry) {
				...
				if (m === 'wall.post') {
					let validAttacheRegexp = /(^https?:\/\/)|(^(poll|album|photo|video|doc|audio|page|note)-?\d+_-?\d+)$/,
						validAttachments = [], methodAccess, queryParams, query, timer;
					...
					for (let i = 0; i < params.attachments.length; i++) {
						let attach = params.attachments[i].trim();
			...
			call: function(method, params, cb, queryTry) {
				let query = params || {}, qs, responseCb;
				...
				if (VK.Api.queryLength(query) < 1500 && !VK.Api.ie6_7()) {
					var useXDM = false;
					let rnd = parseInt(Math.random() * 10000000, 10);
	...
	VK.Auth = {
		...
		login: function(cb, settings) {
			...
			let url = VK._domain.main + '/' + VK._path.login + '?client_id='+VK._apiId+'&display=popup&redirect_uri=close.html&response_type=token&openapi=1';
		...
		revokeGrants: function(cb) {
			...
			if (VK._session && VK._session.sid) {
				let url = VK._domain.login + '?act=openapi&oauth=1&aid=' + parseInt(VK._apiId, 10) + '&location=' + encodeURIComponent(window.location.hostname) + '&do_logout=1&token=' + VK._session.sid;
		...
		getLoginStatus: function(cb, force) {
			...
			let url = VK._domain.login + '?act=openapi&oauth=1&aid=' + parseInt(VK._apiId, 10) + '&location=' + encodeURIComponent(window.location.hostname);
			if (VK.Api.supportCORS()) {
				let loginCallback = function(response) {
					if (!this.JSON) {
						this.JSON = {};
					};
...
VK.UI = {
	...
	_change: function(state, index) {
		let row = document.getElementById('openapi_UI_' + index).rows[0];
		let elems = [row.cells[0].firstChild.firstChild, row.cells[2].firstChild.firstChild];
		for (let i = 0; i < 2; ++i) {
			let elem = elems[i];
			if (state === 0) {
...
VK.XDM = {
	...
	init: function() {
		...
		let url = VK._domain.api + '/' + VK._path.proxy;
		...
		this.remote.append(document.getElementById(VK._rootId), {src: url});
	},
...
VK.Observer = {
	...
	unsubscribe: function(eventName, handler) {
		let subscribers = this._subscribers()[eventName];
...
VK.Util = {
	...
	getStyle: function(elem, name) {
		let ret, defaultView = document.defaultView || window;
		if (defaultView.getComputedStyle) {
			...
			let camelCase = name.replace(/\-(\w)/g, function (all, letter) {
	...
	Box: function(src, sizes, fnc, options) {
		fnc = fnc || {};
		let overflowB = document.body.style.overflow;
		VK.Util.Loader(true);
		let is_vk = /(^|\.)(vk\.com|vkontakte\.ru)$/.test(location.hostname);
		let rpc = new fastXDM.Server(VK.extend(fnc, {
			onInit: function() {
...
VK.Retargeting = {
	...
	Event: function (event) {
		...
		let pData = VK.Util.getPageData();
		let metatagUrl = pData.url.substr(0, 500);
	...
	ProductEvent: function (priceListID, event, params, opts) {
		...
		let pData = VK.Util.getPageData();
		let metatagUrl = pData.url.substr(0, 500);
		let url = 'https://vk.com/rtrg';
		let productParams = params ? JSON.stringify(params) : '';
		let requestParams = {
			'p': this.pixelCode,
		...
		let query = Object.keys(requestParams).map(function(key) {
			let segment = encodeURIComponent(key) + '=' + encodeURIComponent(requestParams[key]);
			return segment;
		}).join('&');
	...
	showErrors: function(errors) {
		...
		let errorBegin = 'VK Pixel Error (' + this.pixelCode + '): ';
...
VK.Goal = function (conversionName, parameters, pixelCode) {
	let pixel = pixelCode || VK.Retargeting.pixelCode;
	...
	let eventName = conversionName || 'conversion';
